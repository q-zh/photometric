...
def calNormalAcc(gt_n, pred_n, mask=None):
    ...
    n_err_mean = ang_valid.sum() / (valid + 1e-5)
    # n_err_med  = ang_valid.median()
    n_acc_11   = (ang_valid < 11.25).sum().float() / (valid + 1e-5)
    n_acc_30   = (ang_valid < 30).sum().float() / (valid + 1e-5)
    n_acc_45   = (ang_valid < 45).sum().float() / (valid + 1e-5)
...
def SphericalDirsToLoc(dirs, width):
...
def SphericalLocToDirs(x_loc, y_loc, width):
    theta = (x_loc.float() / (width - 1)) * 2 - 1
    phi = (y_loc.float() / (width - 1)) * 2 - 1
    delta = torch.sqrt(1 - theta * theta - phi * phi)
    mark = torch.lt(1 - theta * theta - phi * phi,0)
    delta[mark] = 0
    dirs = torch.stack([theta, phi, delta], 1)
    dirs = torch.nn.functional.normalize(dirs, 2, 1)
    return dirs
